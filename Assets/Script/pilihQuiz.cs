...
public class pilihQuiz : MonoBehaviour
{
    ...
    int _info=0;
    string url = "http://192.168.42.199/sinsen/infoStatus.php?pelajaran=";
    string url1 = "http://192.168.42.199/sinsen/cekQuiz.php?nim=";
    ...
    IEnumerator cekQuiz(string nim)
    {
         WWWForm form = new WWWForm();
        form.AddField("nim",nim);
        using (UnityWebRequest www = UnityWebRequest.Post(url1,nim))
        {
            yield return www.SendWebRequest();            
            if (www.isNetworkError || www.isHttpError)
            {
